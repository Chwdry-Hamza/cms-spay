// All views — Dashboard, Blogs, Editor, Analytics, Keywords, Media, Pages, Redirects, Sitemap, Users, Settings

// Reusable bits
const Card = ({ children, style, padding = 18, glow = false }) => (
  <div className={`glass ${glow ? 'glow-border' : ''}`} style={{padding, ...style}}>{children}</div>
);

const StatCard = ({ label, value, delta, deltaDir = 'up', spark, hint }) => (
  <Card padding={18} glow={true} style={{position:'relative', overflow:'hidden'}}>
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom: 10}}>
      <div className="mono" style={{fontSize: 10, color:'var(--text-3)', letterSpacing:'.16em'}}>{label.toUpperCase()}</div>
      {delta && (
    ...
    {spark && <div style={{marginTop: 10, marginLeft: -8, marginRight: -8}}><Sparkline data={spark} width={300} height={36}/></div>}
  </Card>
);
...
const DashboardView = () => {
  ...
  return (
    <div className="content fade-in" style={{display:'flex', flexDirection:'column', gap: 20}}>
      ...
      {/* Main row: traffic chart + score */}
      <div style={{display:'grid', gridTemplateColumns:'2fr 1fr', gap: 14}}>
        <Card padding={20}>
          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: 18}}>
            <div>
              <div className="mono" style={{fontSize: 10, color:'var(--accent-2)', letterSpacing:'.16em', marginBottom: 4}}>SEARCH PERFORMANCE</div>
          ...
          <div style={{display:'flex', gap: 32, marginBottom: 12, alignItems:'baseline'}}>
            <div>
              ...
              <div style={{fontSize: 11, color:'var(--text-2)', display:'flex', gap: 8, alignItems:'center'}}>
                <span className="dot good"/>Visits this period
              </div>
            </div>
          ...
          <div style={{display:'flex', gap: 16, marginTop: 8, fontSize: 11, color:'var(--text-2)'}}>
            <span style={{display:'flex', alignItems:'center', gap: 6}}><span style={{width:10, height:2, background:'var(--accent)', boxShadow:'0 0 8px var(--accent)'}}/>This period</span>
            <span style={{display:'flex', alignItems:'center', gap: 6}}><span style={{width:10, height:1, background:'rgba(255,255,255,.25)', borderTop:'1px dashed rgba(255,255,255,.25)'}}/>Previous</span>
          </div>
        </Card>
            ...
            {[
              ...
              <div key={b.label}>
                ...
                <div style={{height: 4, background:'rgba(255,255,255,.04)', borderRadius: 999}}>
                  <div style={{width: `${b.val}%`, height:'100%', background: `linear-gradient(90deg, ${b.color}, #1ad6db)`, borderRadius: 999, boxShadow:`0 0 8px ${b.color}`}}/>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Keyword rankings + Recent blogs */}
      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr', gap: 14}}>
        <Card padding={20}>
          <SectionHeader eyebrow="TOP KEYWORDS" title="Keyword rankings" right={
            <button className="btn ghost" style={{fontSize: 12}}>View all <Icon name="arrow-right" size={11}/></button>
          }/>
                ...
                <tr key={i} style={{borderTop:'1px solid var(--line)'}}>
                  <td style={{padding:'12px 6px'}}>
                    <div style={{display:'flex', alignItems:'center', gap: 8}}>
                      <Icon name="keywords" size={13} style={{color:'var(--accent-2)', opacity:.7}}/>
                      {r.kw}
                    </div>
                  </td>
                  <td style={{padding:'12px 6px'}}>
        ...
        <Card padding={20}>
          <SectionHeader eyebrow="ACTIVITY" title="Recent blog posts" right={
            <button className="btn ghost" style={{fontSize: 12}}><Icon name="plus" size={12}/>New</button>
          }/>
          <div style={{display:'flex', flexDirection:'column', gap: 12}}>
            {[
              ...
              <div key={i} style={{
                ...
                border:'1px solid var(--line)', background:'rgba(9,14,28,.4)',
                cursor:'pointer', transition:'all .2s'
              }}
              onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(4,186,191,.3)'}
              onMouseLeave={e=>e.currentTarget.style.borderColor='var(--line)'}>
                <div style={{
                  ...
                  <div style={{position:'absolute', inset: 3, borderRadius: 8, background:'var(--bg-2)', display:'grid', placeItems:'center', fontSize: 10, fontWeight: 600}}>{p.score}</div>
                </div>
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize: 12.5, fontWeight: 500, marginBottom: 3, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{p.t}</div>
                  <div style={{fontSize: 11, color:'var(--text-3)', display:'flex', gap: 8, alignItems:'center'}}>
                    <span>{p.a}</span><span>·</span><span>{p.d}</span>
                    <span className={`chip ${p.s==='published'?'good':p.s==='review'?'warn':''}`} style={{marginLeft:'auto', padding:'1px 6px', fontSize: 9}}>{p.s}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Indexing + activity heatmap + traffic by source */}
        ...
        <Card padding={20}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', marginTop: 14, fontSize: 11, color:'var(--text-3)'}}>
            ...
            <div style={{display:'flex', gap: 3}}>
              {[0.05, 0.18, 0.45, 0.75, 1].map((a, i) => <div key={i} style={{width:10, height:10, borderRadius:2, background:`rgba(4,186,191,${a})`}}/>)}
            </div>
            <span>More</span>
          </div>
            ...
            {[
              ...
              <div key={r.r}>
                <div style={{display:'flex', justifyContent:'space-between', fontSize: 11.5, marginBottom: 3}}>
                  ...
                  <span className="mono" style={{color:'#fff'}}>{r.n} <span style={{color:'var(--text-3)'}}>· {r.v}%</span></span>
                </div>
                <div style={{height: 3, background:'rgba(255,255,255,.04)', borderRadius: 999}}>
                  <div style={{width: `${r.v*2}%`, height:'100%', background:'linear-gradient(90deg, var(--accent), var(--accent-2))', borderRadius: 999}}/>
                </div>
              </div>
            ))}
          </div>
